...
events = [{"start": "2018-06-05T01:07:03",
          ...
          {"start": "2018-06-05T04:07:36",
           "stop": "2018-06-05T04:10:36",
           "generation_time": "2019-06-06T13:33:29",
           "values":[
               {"name": "test_object_name1",
                "level_position": 0,
                "parent_level": -1,
...
for info in events_ddbb:
    ...
    if len(event) == 1 and "values" in event[0]:
        ...
        if len(values_ddbb) != len(event[0]["values"]):
            result = {"message":"NOK","color":"red"}
        # end if
        print(colored("Check", on_color="on_blue") + "_{}: The number of values associated to the event inserted {} is the expected {} --> ".format(getframeinfo(currentframe()).lineno, len(values_ddbb), len(event[0]["values"]), value["name"]) + colored(result["message"], result["color"], attrs=['bold']))
        for value in event[0]["values"]:
            value_ddbb = [value_ddbb for value_ddbb in values_ddbb if value_ddbb.level_position == value["level_position"] and value_ddbb.parent_level == value["parent_level"] and value_ddbb.parent_position == value["parent_position"] and value_ddbb.name == value["name"]]
            result = {"message":"OK","color":"green"}
            if len(value_ddbb) != 1:
                result = {"message":"NOK","color":"red"}
...
if number_values != len(values_ddbb):
    result = {"message":"NOK","color":"red"}
# end if
print(colored("Check", on_color="on_blue") + "_{}: The number of values associated to events inserted {} is the expected {} --> ".format(getframeinfo(currentframe()).lineno, number_values, len(values_ddbb), value["name"]) + colored(result["message"], result["color"], attrs=['bold']))
