...
class TestExport(unittest.TestCase):
    ...
    def test_build_values_structure(self):
        """
        Method to test the build_values_structure function through the get_structured_values method
        """
    ...
    def test_export_events_wrong_include_alerts(self):
        """
        Method to test the export_events function with wrong include_alerts argument
        """

        test_success = False
        structure = {}
        group = "group"
        include_alerts = "not_bool"
        try:
            export.export_events(structure, [], group = group, include_alerts = include_alerts)
        except ErrorParsingParameters:
            test_success = True
        # end try
    ...
    def test_export_events(self):
        ...
        # Initialize structure to help with the code coverage analysis
        structure = {"events": {}, "gauges": {}}
        ...
        assert structure == {
            ...
            "explicit_references": {
                str(events[0].explicitRef.explicit_ref_uuid): {
                    ...
                    "events": {"GAUGE_NAME": [{"event_uuid": str(events[0].event_uuid),
                                               "name": "GAUGE_NAME",
                                               "system": "GAUGE_SYSTEM"}]},
                }
            },
    ...
    def test_export_events_duplicated(self):
        """
        Method to test the duplicated calling to export_events function
        """
    ...
    def test_export_events_including_sources(self):
        """
        Method to test the export_events function including sources
        """
        ...
        structure = {}
        export.export_events(structure, events, include_alerts = False, include_sources = True)
        ...
        assert structure == {
            ...
            "sources": {str(sources[0].source_uuid): {"dim_signature": "dim_signature",
                                                                 "generation_time": "2018-01-01T00:00:00",
                                                                 "ingested": True,
                                                                 ...
                                                                 "reception_time": "2018-01-01T00:00:00",
                                                                 "reported_generation_time": "2018-01-01T00:00:00",
                                                                 "reported_validity_start": "2018-01-01T00:00:00",
                                                                 "reported_validity_stop": "2018-01-02T00:00:00",
                                                                 "source_uuid": str(sources[0].source_uuid),
                                                                 "statuses": [{"log": "The source file source1.xml associated to the DIM signature dim_signature and DIM processing exec with version 1.0 is going to be ingested",
                                                                               "source_uuid": str(sources[0].source_uuid),
                                                                               "status": 1,
                                                                               "time_stamp": [status for status in sources[0].statuses if status.status == 1][0].time_stamp.isoformat()},
                                                                              {"log": "The source file source1.xml associated to the DIM signature dim_signature and DIM processing exec with version 1.0 has ingested correctly 1 event/s, 0 annotation/s and 0 alert/s",
                                                                               "source_uuid": str(sources[0].source_uuid),
                                                                               "status": 0,
                                                                               "time_stamp": [status for status in sources[0].statuses if status.status == 0][0].time_stamp.isoformat()}],
                                                                 "validity_start": "2018-01-01T00:00:00",
                                                                 "validity_stop": "2018-01-02T00:00:00"}},
        }
    ...
    def test_export_annotations_wrong_include_events(self):
        """
        Method to test the export_annotations function with wrong include_events argument
        """

        test_success = False
        structure = {}
        group = "group"
        include_events = "not_bool"
        try:
            export.export_annotations(structure, [], group = group, include_events = include_events)
        except ErrorParsingParameters:
            test_success = True
        # end try
    ...
    def test_export_annotations_wrong_include_alerts(self):
        """
        Method to test the export_annotations function with wrong include_alerts argument
        """
        ...
        include_alerts = "not_bool"
        try:
            export.export_annotations(structure, [], group = group, include_alerts = include_alerts)
        except ErrorParsingParameters:
            test_success = True
        # end try
    ...
    def test_export_annotations(self):
        ...
        export.export_annotations(structure, annotations, include_sources = False, include_events = False)
    ...
    def test_export_annotations_duplicated(self):
        """
        Method to test the duplicated calling to export_annotations function
        """
    ...
    def test_export_annotations_with_group(self):
        ...
        structure = {}
        export.export_annotations(structure, annotations, group = "annotation_group", include_sources = False, include_events = False)
    ...
    def test_export_annotations_without_explicit_references(self):
        ...
        structure = {}
        export.export_annotations(structure, annotations, include_ers = False, group = "annotation_group", include_sources = False, include_events = False)
    ...
    def test_export_annotations_including_sources(self):
        """
        Method to test the export_annotations function including sources
        """
        ...
        structure = {}
        export.export_annotations(structure, annotations, include_ers = False, include_events = False, include_alerts = False, include_sources = True)
        ...
        assert structure == {
            ...
            "sources": {str(sources[0].source_uuid): {"dim_signature": "dim_signature",
                                                      ...
                                                      "statuses": [{"log": "The source file source1.xml associated to the DIM signature dim_signature and DIM processing exec with version 1.0 is going to be ingested",
                                                                    ...
                                                                    "status": 1,
                                                                    "time_stamp": [status for status in sources[0].statuses if status.status == 1][0].time_stamp.isoformat()},
                                                                   {"log": "The source file source1.xml associated to the DIM signature dim_signature and DIM processing exec with version 1.0 has ingested correctly 0 event/s, 1 annotation/s and 0 alert/s",
                                                                    "source_uuid": str(sources[0].source_uuid),
                                                                    "status": 0,
                                                                    "time_stamp": [status for status in sources[0].statuses if status.status == 0][0].time_stamp.isoformat()}],
                                                      "validity_start": "2018-01-01T00:00:00",
    ...
    def test_export_annotations_including_alerts(self):
        """
        Method to test the export_annotations function including alerts
        """
        ...
        annotation_alerts = self.query_eboa.get_annotation_alerts()

        assert len(annotation_alerts) == 1

        structure = {}
        export.export_annotations(structure, annotations, include_ers = False, include_events = False, include_alerts = True, include_sources = False)
        ...
        assert structure == {
            "annotations": {
                str(annotations[0].annotation_uuid): {
                    ...
                    "source": {"source_uuid": str(annotations[0].source.source_uuid),
                               "name": "source1.xml"},
                    "alerts": [str(annotation_alerts[0].get_uuid())],
                    "indexed_values": {},
                    "values": []
                }
            },
            "annotation_alerts": {
                str(annotation_alerts[0].get_uuid()): {
                    "annotation_alert_uuid": str(annotation_alerts[0].get_uuid()),
                    "message": "Alert message",
                    "validated": "",
                    "ingestion_time": annotation_alerts[0].ingestion_time.isoformat(),
                    "generator": "test",
                    "notified": "",
                    "solved": "",
                    "solved_time": "",
                    ...
                    "justification": "",
                    "definition": {
                        "alert_uuid": str(annotation_alerts[0].alertDefinition.alert_uuid),
                        "name": "alert_name1",
                        "severity": 4,
                        "description": "Alert description",
                        "group": "alert_group"
            ...
            "annotation_alerts_groups": {
                "alert_group": [str(annotation_alerts[0].get_uuid())]
            }
        }
    ...
    def test_export_ers_wrong_include_events(self):
        """
        Method to test the export_ers function with wrong include_events argument
        """
        ...
        include_events = "not_bool"
        try:
            export.export_ers(structure, [], group = group, include_events = include_events)
        except ErrorParsingParameters:
            test_success = True
        # end try
    ...
    def test_export_ers_wrong_include_alerts(self):
        """
        Method to test the export_ers function with wrong include_alerts argument
        """
        ...
        include_alerts = "not_bool"
        try:
            export.export_ers(structure, [], group = group, include_alerts = include_alerts)
        except ErrorParsingParameters:
            test_success = True
        # end try
        ...
        assert structure == {
            "explicit_references": {
                str(ers[0].explicit_ref_uuid): {
                    ...
                    "events": {
                        "GAUGE_NAME": [{
                            "event_uuid": str(ers[0].events[0].event_uuid),
                            "name": "GAUGE_NAME",
                            "system": "GAUGE_SYSTEM",
                        }]
                    }
            ...
            "events": {
                str(ers[0].events[0].event_uuid): {
                    ...
                    "links_to_me": [],
                    "duration": 3600.0,
                    "ingestion_time": ers[0].events[0].ingestion_time.isoformat(),
                    "gauge": {"gauge_uuid": str(ers[0].events[0].gauge.gauge_uuid),
                              "dim_signature": "dim_signature",
                              "name": "GAUGE_NAME",
                              "system": "GAUGE_SYSTEM",
                              "description": ""},
                    "source": {"source_uuid": str(ers[0].events[0].source.source_uuid),
                               "name": "source1.xml"}
                }
            },
            "gauges": {
                "GAUGE_NAME": {
                    "GAUGE_SYSTEM": [str(ers[0].events[0].event_uuid)]
                }
            },
            "annotations": {
    ...
    def test_export_ers_duplicated(self):
        """
        Method to test the export_ers function with no optional arguments
        """
        ...
        export.export_ers(structure, ers, include_sources = False, include_events = False)
    ...
    def test_export_ers_no_ers(self):
        """
    ...
    def test_export_ers_with_group(self):
        """
        Method to test the export_ers function with group
        """
        ...
        structure = {}
        export.export_ers(structure, ers, include_sources = False, include_events = False, group = "ER_GROUP")

        assert structure == {
            "explicit_reference_groups": {
                "ER_GROUP": [str(ers[0].explicit_ref_uuid)]
            },
            "explicit_references": {
                str(ers[0].explicit_ref_uuid): {
                    "explicit_ref_uuid": str(ers[0].explicit_ref_uuid),
    ...
    def test_export_ers_including_alerts(self):
        """
        Method to test the export_ers function including alerts
        """
        ...
        er_alerts = self.query_eboa.get_explicit_ref_alerts()

        assert len(er_alerts) == 1

        structure = {}
        export.export_ers(structure, ers, include_annotations = False, include_events = False, include_alerts = True, include_sources = False)

        assert structure == {
            "explicit_references": {
                str(ers[0].explicit_ref_uuid): {
                    "alerts": [str(er_alerts[0].get_uuid())],
                    "explicit_ref": "EXPLICIT_REFERENCE",
                    "explicit_ref_uuid": str(ers[0].explicit_ref_uuid),
                    "ingestion_time": ers[0].ingestion_time.isoformat()
                }
            ...
            "explicit_ref_alerts": {
                str(er_alerts[0].get_uuid()): {
                    "explicit_ref_alert_uuid": str(er_alerts[0].get_uuid()),
                    "message": "Alert message",
                    "validated": "",
                    "ingestion_time": er_alerts[0].ingestion_time.isoformat(),
                    "generator": "test",
                    "notified": "",
                    "solved": "",
                    "solved_time": "",
                    ...
                    "justification": "",
                    "definition": {
                        "alert_uuid": str(er_alerts[0].alertDefinition.alert_uuid),
                        "name": "alert_name1",
                        "severity": 4,
                        "description": "Alert description",
                        "group": "alert_group"
            ...
            "explicit_ref_alerts_groups": {
                "alert_group": [str(er_alerts[0].get_uuid())]
            }
        }

    def test_export_sources_wrong_structure(self):
        """
        Method to test the export_sources function with wrong structure argument
        """

        test_success = False
        structure = "not_a_dict"
        try:
            export.export_sources(structure, [])
        except ErrorParsingParameters:
            test_success = True
        # end try
    ...
    def test_export_sources_wrong_group(self):
        """
        Method to test the export_sources function with wrong group argument
        """
        ...
        group = {}
        try:
            export.export_sources(structure, [], group = group)
        except ErrorParsingParameters:
            test_success = True
        # end try
    ...
    def test_export_sources_wrong_include_alerts(self):
        """
        Method to test the export_sources function with wrong include_alerts argument
        """
        ...
        include_alerts = "not_bool"
        try:
            export.export_sources(structure, [], group = group, include_alerts = include_alerts)
        except ErrorParsingParameters:
            test_success = True
        # end try
    ...
    def test_export_sources(self):
        """
        Method to test the export_sources function with no optional arguments
        """
        ...
        # Initialize structure to help with the code coverage analysis
        structure = {"sources": {}}
        export.export_sources(structure, sources)
        ...
        assert structure == {
            "sources": {str(sources[0].source_uuid): {"dim_signature": "dim_signature",
                                                      ...
                                                      "statuses": [{"log": "The source file source1.xml associated to the DIM signature dim_signature and DIM processing exec with version 1.0 is going to be ingested",
                                                                    ...
                                                                    "status": 1,
                                                                    "time_stamp": [status for status in sources[0].statuses if status.status == 1][0].time_stamp.isoformat()},
                                                                   {"log": "The source file source1.xml associated to the DIM signature dim_signature and DIM processing exec with version 1.0 has ingested correctly 0 event/s, 0 annotation/s and 0 alert/s",
                                                                    "source_uuid": str(sources[0].source_uuid),
                                                                    "status": 0,
                                                                    "time_stamp": [status for status in sources[0].statuses if status.status == 0][0].time_stamp.isoformat()}],
                                                      "validity_start": "2018-01-01T00:00:00",
    ...
    def test_export_sources_duplicated(self):
        """
        Method to test the duplicated calling to export_sources function
        """
    ...
    def test_export_sources_no_sources(self):
        """
        Method to test the export_sources function with no optional arguments with no sources in DDBB
        """

        structure = {}
        export.export_sources(structure, [], group = "source_group")
    ...
    def test_export_sources_with_group(self):
        """
        Method to test the export_sources function with group
        """
        ...
        structure = {}
        export.export_sources(structure, sources, group = "source_group")

        assert structure == {
            "source_groups": {
                "source_group": [str(sources[0].source_uuid)]
            },
            "sources": {str(sources[0].source_uuid): {"dim_signature": "dim_signature",
                                                      "generation_time": "2018-01-01T00:00:00",
                                                      "ingested": True,
    ...
    def test_export_sources_including_alerts(self):
        """
        Method to test the export_sources function including alerts
        """
        ...
        structure = {}
        export.export_sources(structure, sources, include_alerts = True)
        ...
        assert structure == {
            "sources": {str(sources[0].source_uuid): {"dim_signature": "dim_signature",
                                                      ...
                                                      "statuses": [{"log": "The source file source1.xml associated to the DIM signature dim_signature and DIM processing exec with version 1.0 is going to be ingested",
                                                                    ...
                                                                    "status": 1,
                                                                    "time_stamp": [status for status in sources[0].statuses if status.status == 1][0].time_stamp.isoformat()},
                                                                   {"log": "The source file source1.xml associated to the DIM signature dim_signature and DIM processing exec with version 1.0 has ingested correctly 0 event/s, 0 annotation/s and 1 alert/s",
                                                                    "source_uuid": str(sources[0].source_uuid),
                                                                    "status": 0,
                                                                    "time_stamp": [status for status in sources[0].statuses if status.status == 0][0].time_stamp.isoformat()}],
                                                      "validity_start": "2018-01-01T00:00:00",
                                                      "validity_stop": "2018-01-02T00:00:00",
                                                      "alerts": [str(source_alerts[0].get_uuid())]
            }
            },
            "source_alerts": {
                str(source_alerts[0].get_uuid()): {
                    "source_alert_uuid": str(source_alerts[0].get_uuid()),
                    "message": "Alert message",
                    "validated": "",
                    "ingestion_time": source_alerts[0].ingestion_time.isoformat(),
                    "generator": "test",
                    "notified": "",
                    "solved": "",
                    "solved_time": "",
                    ...
                    "justification": "",
                    "definition": {
                        "alert_uuid": str(source_alerts[0].alertDefinition.alert_uuid),
                        "name": "alert_name1",
                        "severity": 4,
                        "description": "Alert description",
                        "group": "alert_group"
            ...
            "source_alerts_groups": {
                "alert_group": [str(source_alerts[0].get_uuid())]
            }
        }

    def test_export_alerts_wrong_structure(self):
        """
        Method to test the export_alerts function with wrong structure argument
        """

        test_success = False
        structure = "not_a_dict"
        try:
            export.export_alerts(structure, [])
        except ErrorParsingParameters:
            test_success = True
        # end try
    ...
    def test_export_alerts_wrong_group(self):
        """
        Method to test the export_alerts function with wrong group argument
        """
        ...
        group = {}
        try:
            export.export_alerts(structure, [], group = group)
        except ErrorParsingParameters:
            test_success = True
        # end try
    ...
    def test_export_event_alerts(self):
        ...
        structure = {}
        export.export_alerts(structure, event_alerts, group = "alert_group")
    ...
    def test_export_event_alerts_duplicated(self):
        """
        Method to test the duplicated calling to export_alerts function
        """
    ...
    def test_export_alerts_no_alerts(self):
        """
        Method to test the export_alerts function with no optional arguments with no alerts in DDBB
        """

        structure = {}
        export.export_alerts(structure, [], group = "alert_group")
