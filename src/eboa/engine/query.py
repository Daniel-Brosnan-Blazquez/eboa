...
class Query():
    ...
    def get_source_alerts(self, source_uuids = None, dim_signature_uuids = None, source_names = None, validity_start_filters = None, validity_stop_filters = None, validity_duration_filters = None, reported_validity_start_filters = None, reported_validity_stop_filters = None, reported_validity_duration_filters = None, reception_time_filters = None, generation_time_filters = None, reported_generation_time_filters = None, source_ingestion_time_filters = None, processing_duration_filters = None, ingestion_duration_filters = None, ingestion_completeness = None, ingested = None, ingestion_error = None, processors = None, processor_version_filters = None, dim_signatures = None, statuses = None, names = None, severities = None, groups = None, alert_uuids = None, validated = None, alert_ingestion_time_filters = None, generators = None, notified = None, solved = None, solved_time_filters = None, notification_time_filters = None, order_by = None, limit = None, offset = None, delete = False):
        ...
        :type dim_signature_uuids: text_filter
        :param source_names: source name filters
        :type source_names: text_filter
        :param validity_start_filters: list of validity start filters
        :type validity_start_filters: date_filters
        :param validity_stop_filters: list of validity stop filters
        :type validity_stop_filters: date_filters
        ...
        :param reported_generation_time_filters: list of reported generation time filters
        :type reported_generation_time_filters: date_filters
        :param source_ingestion_time_filters: list of ingestion time filters
        :type source_ingestion_time_filters: date_filters
        :param processing_duration_filters: list of processing duration filters
        :type processing_duration_filters: float_filters
        :param ingestion_duration_filters: list of ingestion duration filters
        :type ingestion_duration_filters: float_filters
        ...
        :param alert_uuids: 
        :type alert_uuids: text_filter
        :param validated: 
        :type validated: boolean_filter
        :param alert_ingestion_time_filters: 
        :type alert_ingestion_time_filters: date_filters
        :param generators: 
        :type generators: text_filter
        :param notified: 
        :type notified: boolean_filter
        :param solved: 
        :type solved: boolean_filter
        :param solved_time_filters: 
        :type solved_time_filters: date_filters
        :param notification_time_filters: 
        :type notification_time_filters: date_filters
        :param order_by: field to order by
        :type order_by: order_by statement
        :param limit: Positive integer to limit the number of results of the query
        :type limit: Positive integer
        ...
        if source_uuids != None:
            ...
            if source_uuids["op"] in arithmetic_operators.keys():
                op = arithmetic_operators[source_uuids["op"]]
                params.append(op(SourceAlert.source_uuid, source_uuids["filter"]))
            else:
                filter = eval('SourceAlert.source_uuid.' + text_operators[source_uuids["op"]])
                params.append(filter(source_uuids["filter"]))
            # end if
        # end if
        ...
        # Source names
        if source_names != None:
            functions.is_valid_text_filter(source_names)
            if source_names["op"] in arithmetic_operators.keys():
                op = arithmetic_operators[source_names["op"]]
                params.append(op(Source.name, source_names["filter"]))
            else:
                filter = eval('Source.name.' + text_operators[source_names["op"]])
                params.append(filter(source_names["filter"]))
            # end if
            join_tables = True
        # end if
        ...
        if source_ingestion_time_filters != None:
            functions.is_valid_date_filters(source_ingestion_time_filters)
            for ingestion_time_filter in source_ingestion_time_filters:
                op = arithmetic_operators[ingestion_time_filter["op"]]
                params.append(op(Source.ingestion_time, ingestion_time_filter["date"]))
            # end for
            join_tables = True
        ...
        # Alert UUIDs
        if alert_uuids != None:
            functions.is_valid_text_filter(alert_uuids)
            if alert_uuids["op"] in arithmetic_operators.keys():
                op = arithmetic_operators[alert_uuids["op"]]
                params.append(op(SourceAlert.alert_uuid, alert_uuids["filter"]))
            else:
                filter = eval('SourceAlert.alert_uuid.' + text_operators[alert_uuids["op"]])
                params.append(filter(alert_uuids["filter"]))
            # end if
        # end if
        
        # validated filter
        if validated != None:
            functions.is_valid_bool_filter(validated)
            params.append(SourceAlert.validated == validated)
        # end if
        
        # source alert ingestion_time filters
        if alert_ingestion_time_filters != None:
            functions.is_valid_date_filters(alert_ingestion_time_filters)
            for ingestion_time_filter in alert_ingestion_time_filters:
                op = arithmetic_operators[ingestion_time_filter["op"]]
        ...
        if generators != None:
            ...
            if generators["op"] in arithmetic_operators.keys():
                ...
                params.append(op(SourceAlert.generator, generators["filter"]))
            else:
                filter = eval('SourceAlert.generator.' + text_operators[generators["op"]])
        ...
        # notified filter
        if notified != None:
            functions.is_valid_bool_filter(notified)
            params.append(SourceAlert.notified == notified)
        # end if

        # solved filter
        if solved != None:
            functions.is_valid_bool_filter(solved)
            params.append(SourceAlert.solved == solved)
        # end if

        # source alert solved time filters
        if solved_time_filters != None:
            functions.is_valid_date_filters(solved_time_filters)
            for solved_time_filter in solved_time_filters:
        ...
        if notification_time_filters != None:
            functions.is_valid_date_filters(notification_time_filters)
            for notification_time_filter in notification_time_filters:
        ...
        if order_by != None:
            functions.is_valid_order_by(order_by)
            if order_by["descending"]:
                order_by_statement = eval("SourceAlert." + order_by["field"] + ".desc()")
            else:
                order_by_statement = eval("SourceAlert." + order_by["field"])
            # end if
            query = query.order_by(order_by_statement)
        # end if
    ...
    def get_report_alerts(self, report_uuids = None, report_group_uuids = None, report_names = None, generation_modes = None, validity_start_filters = None, validity_stop_filters = None, validity_duration_filters = None, triggering_time_filters = None, generation_start_filters = None, generation_stop_filters = None, generated = None, compressed = None, generation_error = None, report_generators_filters = None, generator_version_filters = None, statuses = None, report_groups = None, names = None, severities = None, groups = None, alert_uuids = None, validated = None, alert_ingestion_time_filters = None, generators = None, notified = None, solved = None, solved_time_filters = None, notification_time_filters = None, order_by = None, limit = None, offset = None, delete = None):
        ...
        if report_uuids != None:
            ...
            if report_uuids["op"] in arithmetic_operators.keys():
                op = arithmetic_operators[report_uuids["op"]]
                params.append(op(ReportAlert.report_uuid, report_uuids["filter"]))
            else:
                filter = eval('ReportAlert.report_uuid.' + text_operators[report_uuids["op"]])
                params.append(filter(report_uuids["filter"]))
            # end if
        # end if
        ...
        # Report names
        if report_names != None:
            functions.is_valid_text_filter(report_names)
            if report_names["op"] in arithmetic_operators.keys():
                op = arithmetic_operators[report_names["op"]]
                params.append(op(Report.name, report_names["filter"]))
            else:
                filter = eval('Report.name.' + text_operators[report_names["op"]])
                params.append(filter(report_names["filter"]))
            # end if
            join_tables = True
        # end if
        ...
        # Generators
        if report_generators_filters != None:
            functions.is_valid_text_filter(report_generators_filters)
            if report_generators_filters["op"] in arithmetic_operators.keys():
                op = arithmetic_operators[report_generators_filters["op"]]
                params.append(op(Report.generator, report_generators_filters["filter"]))
            else:
                filter = eval('Report.generator.' + text_operators[report_generators_filters["op"]])
                params.append(filter(report_generators_filters["filter"]))
            # end if
            join_tables = True
        # end if
        ...
        if alert_uuids != None:
            ...
            if alert_uuids["op"] in arithmetic_operators.keys():
                op = arithmetic_operators[alert_uuids["op"]]
                params.append(op(ReportAlert.alert_uuid, alert_uuids["filter"]))
            else:
                filter = eval('ReportAlert.alert_uuid.' + text_operators[alert_uuids["op"]])
                params.append(filter(alert_uuids["filter"]))
            # end if
        # end if
        ...
        if validated != None:
            functions.is_valid_bool_filter(validated)
            params.append(ReportAlert.validated == validated)
        # end if
        ...
        if generators != None:
            ...
            if generators["op"] in arithmetic_operators.keys():
                op = arithmetic_operators[generators["op"]]
                params.append(op(ReportAlert.generator, generators["filter"]))
            else:
                filter = eval('ReportAlert.generator.' + text_operators[generators["op"]])
                params.append(filter(generators["filter"]))
            # end if
        # end if
        ...
        if notified != None:
            functions.is_valid_bool_filter(notified)
            params.append(ReportAlert.notified == notified)
        # end if

        # solved filter
        if solved != None:
            functions.is_valid_bool_filter(solved)
            params.append(ReportAlert.solved == solved)
        # end if
        ...
        if order_by != None:
            functions.is_valid_order_by(order_by)
            if order_by["descending"]:
                order_by_statement = eval("ReportAlert." + order_by["field"] + ".desc()")
            else:
                order_by_statement = eval("ReportAlert." + order_by["field"])
            # end if
            query = query.order_by(order_by_statement)
        # end if
    ...
    def get_event_alerts(self, event_uuids = None, source_uuids = None, explicit_ref_uuids = None, gauge_uuids = None, sources = None, explicit_refs = None, gauge_names = None, gauge_systems = None, keys = None, start_filters = None, stop_filters = None, duration_filters = None, event_ingestion_time_filters = None, value_filters = None, names = None, severities = None, groups = None, alert_uuids = None, validated = None, alert_ingestion_time_filters = None, generators = None, notified = None, solved = None, solved_time_filters = None, notification_time_filters = None, order_by = None, limit = None, offset = None, delete = None):
        ...
        if event_uuids != None:
            ...
            if event_uuids["op"] in arithmetic_operators.keys():
                op = arithmetic_operators[event_uuids["op"]]
                params.append(op(EventAlert.event_uuid, event_uuids["filter"]))
            else:
                filter = eval('EventAlert.event_uuid.' + text_operators[event_uuids["op"]])
                params.append(filter(event_uuids["filter"]))
            # end if
        # end if
        ...
        if value_filters != None:
            tables_values = []
            self.prepare_query_values(value_filters, event_value_entities, params, tables_values)
        ...
        if alert_uuids != None:
            ...
            if alert_uuids["op"] in arithmetic_operators.keys():
                op = arithmetic_operators[alert_uuids["op"]]
                params.append(op(EventAlert.alert_uuid, alert_uuids["filter"]))
            else:
                filter = eval('EventAlert.alert_uuid.' + text_operators[alert_uuids["op"]])
                params.append(filter(alert_uuids["filter"]))
            # end if
        # end if
        ...
        if validated != None:
            functions.is_valid_bool_filter(validated)
            params.append(EventAlert.validated == validated)
        # end if
        ...
        if generators != None:
            ...
            if generators["op"] in arithmetic_operators.keys():
                op = arithmetic_operators[generators["op"]]
                params.append(op(EventAlert.generator, generators["filter"]))
            else:
                filter = eval('EventAlert.generator.' + text_operators[generators["op"]])
                params.append(filter(generators["filter"]))
            # end if
        # end if
        ...
        if notified != None:
            functions.is_valid_bool_filter(notified)
            params.append(EventAlert.notified == notified)
        # end if

        # solved filter
        if solved != None:
            functions.is_valid_bool_filter(solved)
            params.append(EventAlert.solved == solved)
        # end if
        ...
        if order_by != None:
            functions.is_valid_order_by(order_by)
            if order_by["descending"]:
                order_by_statement = eval("EventAlert." + order_by["field"] + ".desc()")
            else:
                order_by_statement = eval("EventAlert." + order_by["field"])
            # end if
            query = query.order_by(order_by_statement)
        # end if
    ...
    def get_annotation_alerts(self, annotation_uuids = None, source_uuids = None, explicit_ref_uuids = None, annotation_cnf_uuids = None, annotation_ingestion_time_filters = None, sources = None, explicit_refs = None, annotation_cnf_names = None, annotation_cnf_systems = None, value_filters = None, names = None, severities = None, groups = None, alert_uuids = None, validated = None, alert_ingestion_time_filters = None, generators = None, notified = None, solved = None, solved_time_filters = None, notification_time_filters = None, order_by = None, limit = None, offset = None, delete = None):
        ...
        if annotation_uuids != None:
            ...
            if annotation_uuids["op"] in arithmetic_operators.keys():
                op = arithmetic_operators[annotation_uuids["op"]]
                params.append(op(AnnotationAlert.annotation_uuid, annotation_uuids["filter"]))
            else:
                filter = eval('AnnotationAlert.annotation_uuid.' + text_operators[annotation_uuids["op"]])
                params.append(filter(annotation_uuids["filter"]))
            # end if
        # end if
        ...
        if value_filters != None:
            tables_values = []
            self.prepare_query_values(value_filters, annotation_value_entities, params, tables_values)
            for table in set(tables_values):
        ...
        if alert_uuids != None:
            ...
            if alert_uuids["op"] in arithmetic_operators.keys():
                op = arithmetic_operators[alert_uuids["op"]]
                params.append(op(AnnotationAlert.alert_uuid, alert_uuids["filter"]))
            else:
                filter = eval('AnnotationAlert.alert_uuid.' + text_operators[alert_uuids["op"]])
                params.append(filter(alert_uuids["filter"]))
            # end if
        # end if
        ...
        if validated != None:
            functions.is_valid_bool_filter(validated)
            params.append(AnnotationAlert.validated == validated)
        # end if
        ...
        if generators != None:
            ...
            if generators["op"] in arithmetic_operators.keys():
                op = arithmetic_operators[generators["op"]]
                params.append(op(AnnotationAlert.generator, generators["filter"]))
            else:
                filter = eval('AnnotationAlert.generator.' + text_operators[generators["op"]])
                params.append(filter(generators["filter"]))
            # end if
        # end if
        ...
        if notified != None:
            functions.is_valid_bool_filter(notified)
            params.append(AnnotationAlert.notified == notified)
        # end if

        # solved filter
        if solved != None:
            functions.is_valid_bool_filter(solved)
            params.append(AnnotationAlert.solved == solved)
        # end if
        ...
        if order_by != None:
            functions.is_valid_order_by(order_by)
            if order_by["descending"]:
                order_by_statement = eval("AnnotationAlert." + order_by["field"] + ".desc()")
            else:
                order_by_statement = eval("AnnotationAlert." + order_by["field"])
            # end if
            query = query.order_by(order_by_statement)
        # end if
    ...
    def get_explicit_ref_alerts(self, explicit_ref_uuids = None, explicit_ref_group_ids = None, explicit_refs = None, explicit_ref_groups = None, explicit_ref_ingestion_time_filters = None, event_uuids = None, source_uuids = None, sources = None, gauge_uuids = None, gauge_names = None, gauge_systems = None, keys = None, start_filters = None, stop_filters = None, duration_filters = None, event_ingestion_time_filters = None, event_value_filters = None, annotation_uuids = None, annotation_cnf_uuids = None, annotation_ingestion_time_filters = None, annotation_cnf_names = None, annotation_cnf_systems = None, annotation_value_filters = None, names = None, severities = None, groups = None, alert_uuids = None, validated = None, alert_ingestion_time_filters = None, generators = None, notified = None, solved = None, solved_time_filters = None, notification_time_filters = None, order_by = None, limit = None, offset = None, delete = None):
        ...
        if explicit_ref_uuids != None:
            ...
            if explicit_ref_uuids["op"] in arithmetic_operators.keys():
                op = arithmetic_operators[explicit_ref_uuids["op"]]
                params.append(op(ExplicitRefAlert.explicit_ref_uuid, explicit_ref_uuids["filter"]))
            else:
                filter = eval('ExplicitRefAlert.explicit_ref_uuid.' + text_operators[explicit_ref_uuids["op"]])
                params.append(filter(explicit_ref_uuids["filter"]))
            # end if
        # end if
        
        # explicit_ref_group_ids
        if explicit_ref_group_ids != None:
            functions.is_valid_text_filter(explicit_ref_group_ids)
            if explicit_ref_group_ids["op"] in arithmetic_operators.keys():
                op = arithmetic_operators[explicit_ref_group_ids["op"]]
                params.append(op(ExplicitRef.expl_ref_cnf_uuid, explicit_ref_group_ids["filter"]))
            else:
                filter = eval('ExplicitRef.expl_ref_cnf_uuid.' + text_operators[explicit_ref_group_ids["op"]])
                params.append(filter(explicit_ref_group_ids["filter"]))
            # end if
            join_tables = True
        # end if
        ...
        # Groups
        if explicit_ref_groups != None:
            functions.is_valid_text_filter(explicit_ref_groups)
            if explicit_ref_groups["op"] in arithmetic_operators.keys():
                op = arithmetic_operators[explicit_ref_groups["op"]]
                params.append(op(ExplicitRefGrp.name, explicit_ref_groups["filter"]))
            else:
                filter = eval('ExplicitRefGrp.name.' + text_operators[explicit_ref_groups["op"]])
                params.append(filter(explicit_ref_groups["filter"]))
            # end if
            join_tables = True
            tables[ExplicitRefGrp] = ExplicitRefGrp.expl_ref_cnf_uuid==ExplicitRef.expl_ref_cnf_uuid
        # end if
        ...
        if alert_uuids != None:
            ...
            if alert_uuids["op"] in arithmetic_operators.keys():
                op = arithmetic_operators[alert_uuids["op"]]
                params.append(op(ExplicitRefAlert.alert_uuid, alert_uuids["filter"]))
            else:
                filter = eval('ExplicitRefAlert.alert_uuid.' + text_operators[alert_uuids["op"]])
                params.append(filter(alert_uuids["filter"]))
            # end if
        # end if
        ...
        if validated != None:
            functions.is_valid_bool_filter(validated)
            params.append(ExplicitRefAlert.validated == validated)
        # end if
        ...
        if generators != None:
            ...
            if generators["op"] in arithmetic_operators.keys():
                op = arithmetic_operators[generators["op"]]
                params.append(op(ExplicitRefAlert.generator, generators["filter"]))
            else:
                filter = eval('ExplicitRefAlert.generator.' + text_operators[generators["op"]])
                params.append(filter(generators["filter"]))
            # end if
        # end if
        ...
        if notified != None:
            functions.is_valid_bool_filter(notified)
            params.append(ExplicitRefAlert.notified == notified)
        # end if

        # solved filter
        if solved != None:
            functions.is_valid_bool_filter(solved)
            params.append(ExplicitRefAlert.solved == solved)
        # end if
        ...
        if order_by != None:
            functions.is_valid_order_by(order_by)
            if order_by["descending"]:
                order_by_statement = eval("ExplicitRefAlert." + order_by["field"] + ".desc()")
            else:
                order_by_statement = eval("ExplicitRefAlert." + order_by["field"])
            # end if
            query = query.order_by(order_by_statement)
        # end if
