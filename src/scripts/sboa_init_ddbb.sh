...
DATABASE=`psql -p "$PORT" -h "$HOST" -t -U postgres -c "SELECT count(*) FROM pg_database WHERE datname='sboadb'";`
if [ $DATABASE -eq 1 ];
then
    CONNECTIONS=`psql -p "$PORT" -h "$HOST" -U postgres -t -c "SELECT count(*) FROM pg_stat_activity where datname = 'sboadb';"`
    if [ $CONNECTIONS -ne 0 ];
    then
...
then
    # Drop the DDBB
    psql -p $PORT -h $HOST -U postgres -c "DROP DATABASE sboadb;"

    # Drop the sboa role
    psql -p $PORT -h $HOST -U postgres -c "DROP ROLE sboa;"
fi

# Create DDBB
psql -p $PORT -h $HOST -U postgres -c "CREATE DATABASE sboadb;"

# Fill DDBB
psql -p $PORT -h $HOST -U postgres -d sboadb -f $DATAMODEL_FILE
status=$?
