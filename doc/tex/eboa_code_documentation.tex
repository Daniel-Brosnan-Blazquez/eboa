...
\begin{fulllineitems}
\phantomsection\label{\detokenize{eboa.engine:eboa.engine.engine.Engine.exit_codes}}\pysigline{\sphinxbfcode{\sphinxupquote{exit\_codes}}\sphinxbfcode{\sphinxupquote{ = \{'DUPLICATED\_EVENT\_LINK\_REF': \{'status': 11, 'message': 'The source file with name \{\} associated to the DIM signature \{\} and DIM processing \{\} with version \{\} contains more than one event which defines the same link reference identifier \{\}'\}, 'EVENT\_PERIOD\_NOT\_IN\_SOURCE\_PERIOD': \{'status': 5, 'message': 'The source file with name \{\} associated to the DIM signature \{\} and DIM processing \{\} with version \{\} contains an event with a period (\{\}\_\{\}) outside the period of the source (\{\}\_\{\})'\}, 'FILE\_NOT\_VALID': \{'status': 9, 'message': 'The source file with name \{\} does not pass the schema verification'\}, 'INGESTION\_STARTED': \{'status': 1, 'message': 'The source file \{\} associated to the DIM signature \{\} and DIM processing \{\} with version \{\} is going to be ingested'\}, 'LINKS\_INCONSISTENCY': \{'status': 12, 'message': 'The source file with name \{\} associated to the DIM signature \{\} and DIM processing \{\} with version \{\} defines links between events which lead to clashing unique values into the DDBB. The exception raised has been the following: \{\}'\}, 'ODD\_NUMBER\_OF\_COORDINATES': \{'status': 8, 'message': 'The source file with name \{\} associated to the DIM signature \{\} and DIM processing \{\} with version \{\} contains an event/annotation which defines the geometry value \{\} with an odd number of coordinates'\}, 'OK': \{'status': 0, 'message': 'The source file \{\} associated to the DIM signature \{\} and DIM processing \{\} with version \{\} has ingested correctly \{\} event/s and \{\} annotation/s'\}, 'SOURCE\_ALREADY\_INGESTED': \{'status': 2, 'message': 'The source file \{\} associated to the DIM signature \{\} and DIM processing \{\} with version \{\} has been already ingested'\}, 'UNDEFINED\_EVENT\_LINK\_REF': \{'status': 6, 'message': 'The source file with name \{\} associated to the DIM signature \{\} and DIM processing \{\} with version \{\} contains an event which links to an undefined reference identifier \{\}'\}, 'WRONG\_EVENT\_PERIOD': \{'status': 4, 'message': 'The source file with name \{\} associated to the DIM signature \{\} and DIM processing \{\} with version \{\} contains an event with a stop value \{\} lower than its start value \{\}'\}, 'WRONG\_GEOMETRY': \{'status': 10, 'message': 'The source file with name \{\} associated to the DIM signature \{\} and DIM processing \{\} with version \{\} contains an event/annotation which defines a wrong geometry. The exception raised has been the following: \{\}'\}, 'WRONG\_SOURCE\_PERIOD': \{'status': 3, 'message': 'The source file with name \{\} associated to the DIM signature \{\} and DIM processing \{\} with version \{\} has a validity period which its stop (\{\}) is lower than its start (\{\})'\}, 'WRONG\_VALUE': \{'status': 7, 'message': 'The source file with name \{\} associated to the DIM signature \{\} and DIM processing \{\} with version \{\} contains an event/annotation which defines the value \{\} that cannot be converted to the specified type \{\}'\}\}}}}
\end{fulllineitems}
